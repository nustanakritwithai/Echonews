"""Fixed-role connection pool for c5c.5a signed repeat-login."""
...
SERVICE = "echo_repeat_login_service"
RUNTIME = "echo_repeat_login_runtime"
GUARD = "echo_repeat_login_guard"
FUNCTION = "echo_identity.runtime_repeat_login_session(text,text,text,bigint,bigint,bigint)"


class RepeatLoginPoolError(Exception):
    def __init__(self, code: str):
        self.code = code
        super().__init__(code)


class RepeatLoginPool:
    def __init__(self, *, host, port, dbname, user, password, sslmode="verify-full",
        ...
        if (type(host) is not str or not host or "," in host
            ...
            raise RepeatLoginPoolError("INVALID_SERVICE_CONFIG")
        ...
        if sslmode != "verify-full" and not (insecure and sslmode == "disable"):
            raise RepeatLoginPoolError("VERIFIED_TLS_REQUIRED")
        self._dbname = dbname
        kwargs = dict(
        ...
        self._pool = ConnectionPool(
            ...
            max_waiting=8, timeout=3, open=False, name="echo-repeat-login",
        )
    ...
    def _validate_authority(self, c):
        if c.info.user != SERVICE or c.execute(
            ...
            raise RepeatLoginPoolError("SERVICE_LOGIN_REQUIRED")
        ...
        if roles != [
            ...
            raise RepeatLoginPoolError("ROLE_POLICY_DRIFT")
        ...
        if memberships != [(SERVICE,RUNTIME,False,False,True)]:
            raise RepeatLoginPoolError("ROLE_MEMBERSHIP_DRIFT")
        ...
        if unsafe:
            raise RepeatLoginPoolError("DIRECT_TABLE_PRIVILEGE_DRIFT")
        ...
        if allowed != [(FUNCTION,)]:
            raise RepeatLoginPoolError("FUNCTION_PRIVILEGE_DRIFT")
    ...
    def _sanitize(self, c):
        ...
        if c.info.transaction_status != TransactionStatus.IDLE:
            raise RepeatLoginPoolError("NONIDLE_POOL_BASELINE")
    ...
    def connection(self):
        try:
            ...
            raise RepeatLoginPoolError("SERVICE_CONNECTION_UNAVAILABLE") from None
        body_failed = False
        cleanup_failed = False
        try:
            try:
                self._sanitize(c)
            except RepeatLoginPoolError:
                c.close()
                raise
            except Exception:
                c.close()
                raise RepeatLoginPoolError("POOL_RESET_FAILED") from None
            with c.transaction():
                c.execute("SET LOCAL ROLE echo_repeat_login_runtime", prepare=False)
                if c.execute("SELECT session_user,current_user", prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise RepeatLoginPoolError("LEASE_ROLE_REQUIRED")
                yield c
                if c.closed or c.info.transaction_status != TransactionStatus.INTRANS:
                    raise RepeatLoginPoolError("LEASE_TRANSACTION_CHANGED")
                if c.execute("SELECT session_user,current_user", prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise RepeatLoginPoolError("LEASE_ROLE_CHANGED")
