"""Fixed-role connection pool for the c5c.3 first-session boundary."""
...
GUARD = "echo_first_session_guard"
FUNCTION = "echo_identity.runtime_bootstrap_first_session(text,text,text,bigint,bigint,bigint)"


class FirstSessionPoolError(Exception):
    def __init__(self, code: str):
        self.code = code
        super().__init__(code)


class FirstSessionPool:
    def __init__(self, *, host, port, dbname, user, password, sslmode="verify-full",
        ...
        if (type(host) is not str or not host or "," in host
            ...
            raise FirstSessionPoolError("INVALID_SERVICE_CONFIG")
        ...
        if sslmode != "verify-full" and not (insecure and sslmode == "disable"):
            raise FirstSessionPoolError("VERIFIED_TLS_REQUIRED")
        self._dbname = dbname
        kwargs = dict(
        ...
        self._pool = ConnectionPool(
            ...
            max_waiting=8, timeout=3, open=False, name="echo-first-session",
        )
    ...
    def _validate_authority(self, c):
        if c.info.user != SERVICE or c.execute(
            ...
            raise FirstSessionPoolError("SERVICE_LOGIN_REQUIRED")
        ...
        if roles != [
            ...
            raise FirstSessionPoolError("ROLE_POLICY_DRIFT")
        ...
        if memberships != [(SERVICE,RUNTIME,False,False,True)]:
            raise FirstSessionPoolError("ROLE_MEMBERSHIP_DRIFT")
        ...
        if unsafe:
            raise FirstSessionPoolError("DIRECT_TABLE_PRIVILEGE_DRIFT")
        ...
        if allowed != [(FUNCTION,)]:
            raise FirstSessionPoolError("FUNCTION_PRIVILEGE_DRIFT")
        ...
        if direct or create or sealed is None or sealed[0:2] != (GUARD, True):
            raise FirstSessionPoolError("SERVICE_PRIVILEGE_DRIFT")
        if set(sealed[2] or []) != {"search_path=pg_catalog, pg_temp","row_security=on"}:
            raise FirstSessionPoolError("FUNCTION_CONFIG_DRIFT")

    def _sanitize(self, c):
        if c.closed or c.info.transaction_status == TransactionStatus.UNKNOWN:
            raise FirstSessionPoolError("BROKEN_SERVICE_CONNECTION")
        ...
        if c.info.transaction_status != TransactionStatus.IDLE:
            raise FirstSessionPoolError("NONIDLE_POOL_BASELINE")
    ...
    def connection(self):
        try:
            ...
            raise FirstSessionPoolError("SERVICE_CONNECTION_UNAVAILABLE") from None
        body_failed = False
        cleanup_failed = False
        try:
            try:
                self._sanitize(c)
            except FirstSessionPoolError:
                c.close()
                raise
            except Exception:
                c.close()
                raise FirstSessionPoolError("POOL_RESET_FAILED") from None
            with c.transaction():
                c.execute("SET LOCAL ROLE echo_first_session_runtime", prepare=False)
                if c.execute("SELECT session_user,current_user", prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise FirstSessionPoolError("LEASE_ROLE_REQUIRED")
                yield c
                if c.closed or c.info.transaction_status != TransactionStatus.INTRANS:
                    raise FirstSessionPoolError("LEASE_TRANSACTION_CHANGED")
                if c.execute("SELECT session_user,current_user", prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise FirstSessionPoolError("LEASE_ROLE_CHANGED")
