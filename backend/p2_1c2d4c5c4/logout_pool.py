"""c5c.4 dedicated logout pool. Mirrors the established first-session protocol.
No generic caller-selected role/SQL endpoint and no change to existing pools.
"""
...
SERVICE = 'echo_session_logout_service'
RUNTIME = 'echo_session_logout_runtime'
GUARD = 'echo_session_logout_guard'
FUNCTION = 'echo_identity.runtime_logout_current_session(text,text,text,bigint,bigint,bigint)'


class LogoutPoolError(Exception):
...
class LogoutPool:
    def __init__(self, *, host, port, dbname, user, password, sslmode='verify-full',
        ...
        if (type(host) is not str or not host or ',' in host
            ...
            raise LogoutPoolError('INVALID_SERVICE_CONFIG')
        ...
        if sslmode != 'verify-full' and not (insecure and sslmode == 'disable'):
            raise LogoutPoolError('VERIFIED_TLS_REQUIRED')
        self._dbname = dbname
        ...
        self._pool = ConnectionPool(conninfo='', kwargs=kwargs, min_size=0, max_size=max_size,
            max_waiting=8, timeout=3, open=False, name='echo-session-logout')
    ...
    def _validate_authority(self, c):
        if c.info.user != SERVICE or c.execute('SELECT session_user,current_user,current_database()',
            ...
            raise LogoutPoolError('SERVICE_LOGIN_REQUIRED')
        ...
        if roles != [(GUARD,False,False,False,False,False,False,False),
            ...
            raise LogoutPoolError('ROLE_POLICY_DRIFT')
        ...
        if memberships != [(SERVICE,RUNTIME,False,False,True)]:
            raise LogoutPoolError('ROLE_MEMBERSHIP_DRIFT')
        ...
        if unsafe:
            raise LogoutPoolError('DIRECT_TABLE_PRIVILEGE_DRIFT')
        ...
        if allowed != [(FUNCTION,)]:
            raise LogoutPoolError('FUNCTION_PRIVILEGE_DRIFT')
        ...
        create = c.execute('''SELECT EXISTS(SELECT 1 FROM unnest(%s::text[]) r(name) WHERE
            pg_catalog.has_database_privilege(r.name,current_database(),'CREATE') OR EXISTS(
              SELECT 1 FROM pg_catalog.pg_namespace n WHERE n.nspname=ANY(%s)
              AND pg_catalog.has_schema_privilege(r.name,n.oid,'CREATE')))''',
            ([SERVICE,RUNTIME],['echo_identity','echo_core','public']),prepare=False).fetchone()[0]
        # Catalog comparison avoids resolving an input function name through a
        # schema which the baseline service deliberately cannot access.
    ...
    def _sanitize(self, c):
        ...
        if c.info.transaction_status != TransactionStatus.IDLE:
            raise LogoutPoolError('NONIDLE_POOL_BASELINE')
    ...
    def connection(self):
        try:
            ...
            raise LogoutPoolError('SERVICE_CONNECTION_UNAVAILABLE') from None
        body_failed = cleanup_failed = False
        try:
            try:
                self._sanitize(c)
            except LogoutPoolError:
                c.close()
                raise
            except Exception:
                c.close()
                raise LogoutPoolError('POOL_RESET_FAILED') from None
            with c.transaction():
                c.execute('SET LOCAL ROLE echo_session_logout_runtime',prepare=False)
                if c.execute('SELECT session_user,current_user',prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise LogoutPoolError('LEASE_ROLE_REQUIRED')
                yield c
                if c.closed or c.info.transaction_status != TransactionStatus.INTRANS:
                    raise LogoutPoolError('LEASE_TRANSACTION_CHANGED')
                if c.execute('SELECT session_user,current_user',prepare=False).fetchone() != (SERVICE,RUNTIME):
                    raise LogoutPoolError('LEASE_ROLE_CHANGED')
